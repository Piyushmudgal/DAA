...
void activitySelectionRecursive(vector<activity>& v, vector<activity>& res, int i){
    activity a;
    a = v[i++];
    res.push_back(a);
    // cout<<a;
    while(i < v.size() && v[i].start_time < a.end_time){
        i++;
    }
    if(i < v.size()){
        activitySelectionRecursive(v, res, i);
    }
    else
        return;
}
int main(){
    ...
    cout<<endl;
    vector<activity> res;
    activitySelectionRecursive(v, res, 0);
    for(auto i:res){
        cout<<i;
    }
    cout<<endl;
